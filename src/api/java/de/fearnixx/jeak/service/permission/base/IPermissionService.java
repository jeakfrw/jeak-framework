 ...
 * This can be used for plugins that require more specialized checks where
 * {@link ISubject}, {@link IGroup} and {@link ITS3Subject} are not sufficient.
 */
public interface IPermissionService {

    /**
     * Optionally returns a permission provider for the specified system id.
     * At the moment, the only one automatically registered is
     * {@link de.fearnixx.jeak.profile.IUserIdentity#SERVICE_TEAMSPEAK}.
     */
    Optional<IPermissionProvider> provide(String systemID);

    /**
     * Registers a new provider for other possible systems.
     * This allows external services to be connected into the framework
     * and connector plugins to perform permission checks against them.
     */
     ...
     * @implNote exactly the same as using {@link #provide(String)}
     * with {@link de.fearnixx.jeak.profile.IUserIdentity#SERVICE_TEAMSPEAK}.
     */
